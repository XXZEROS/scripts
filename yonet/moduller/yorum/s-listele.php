...
$degistiz = mysql_query("UPDATE $tablo SET menugos='0' WHERE id='".$_GET['id']."'"); 
 ...
 echo 'cope gitti<br />';
$menuler2 = mysql_query ("select * from yorum where id='".$_GET['id']."' and durum='1'");
while ($aw = mysql_fetch_array($menuler2))  { $gitid=$aw['itid'];
	$yr=0; $yyy = mysql_query ("select id,yorum from itiraf where id='".$gitid."'");
	while ($y = mysql_fetch_array($yyy))  { $yr=$y['yorum']; } $yr = ($yr-1);
	$wwww = mysql_query("UPDATE itiraf SET yorum='".$yr."' WHERE id='".$gitid."'");
	echo 'eksi '.$yr;
  }
...
$menuler2 = mysql_query ("select * from yorum where id='".$_GET['id']."' and durum='2'");
while ($aw = mysql_fetch_array($menuler2))  { $gitid=$aw['itid'];
	$yr=0; $yyy = mysql_query ("select id,yorum from itiraf where id='".$gitid."'");
	while ($y = mysql_fetch_array($yyy))  { $yr=$y['yorum']; } $yr = ($yr+1);
	$wwww = mysql_query("UPDATE itiraf SET yorum='".$yr."' WHERE id='".$gitid."'");
  ...
  $menuler2 = mysql_query ("select * from yorum where id='".$_GET['id']."' and durum='0'");
while ($aw = mysql_fetch_array($menuler2))  { $gitid=$aw['itid']; 
	$yr=0; $yyy = mysql_query ("select id,yorum from itiraf where id='".$gitid."'");
	while ($y = mysql_fetch_array($yyy))  { $yr=$y['yorum']; } $yr = ($yr+1);
	$wwww = mysql_query("UPDATE itiraf SET yorum='".$yr."' WHERE id='".$gitid."'");
...
<table width="927" border="0" cellpadding="6" cellspacing="1" class="tabe">
      <tr>
        <td width="216" bgcolor="#E8E8E8"><strong>Yazar</strong></td>
        <td width="451" bgcolor="#E8E8E8" ><strong>Yorumu</strong></td>
        <td width="148" bgcolor="#E8E8E8"><strong>Cevap olarak</strong></td>
        <td width="26" bgcolor="#E8E8E8"><strong>Düz</strong></td>
        <td width="18" bgcolor="#E8E8E8"><strong>Sil</strong></td>
        </tr>
      <?php 
...
if($c==2) { $dos="kadin.png"; }
  ...
  $tuye=$urun['uyeid']; if($tuye<"1") { echo $urun['rumuz']. ' - <img src="../../../images/'.$dos.'" width="16" height="16" align="bottom" />'; ?><br /><?=$urun['mail'];?><? } else { print auye($tuye);   } ?><br />
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><a href="<?=$linkduz;?>?ID=<?=$urun['id'];?>&dili=<?=$_GET['dili'];?>" class="z"><?php echo $urun['yorum'];?></a></td>
            </tr>
          <tr>
            <td height="28"><? if($urun['durum']==1) { ?><a href="?o=k&id=<?=$urun['id'];?>" class="link">Onayı Kaldır</a><? } else { ?><a href="?o=e&id=<?=$urun['id'];?>" class="link">Onayla</a><? } ?> | <span class="z">Tarih: <?php echo $urun['tarih'];?> | <? if($urun['durum']==2) { ?><a href="?o=e&id=<?=$urun['id'];?>" class="link">Çöpten çıkar ıyk.s</a><? } else { ?><a href="?o=c&id=<?=$urun['id'];?>" class="link">Çöpe fırtlat</a><? } ?></span></td>
            </tr>
        </table></td>
        <td><center>
          <a href="../../../index.php?ney=itiraf&id=<?=$urun['itid'];?>" target="_blank">
        <?=ait($urun['itid']);?>
