...
$hangiid=$_GET['ID'];
$degistir = mysql_query("UPDATE itiraf SET rumuz='".$_POST['rumuz']."', mail='".$_POST['mail']."', cinsiyet='".$_POST['cinsiyet']."', itiraf='".$_POST['yorum']."', baslik='".$_POST['baslik']."'
WHERE id='".$hangiid."'"); 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabe">
  ...
  <tr bgcolor="#F7F7F7">
   <? $akt = mysql_query ("select * from itiraf where id = '".$_GET['ID']."'");
while ($x = mysql_fetch_array($akt)) {  ?><td width="356"><table width="356" border="0" cellspacing="0" cellpadding="0">
     <? if($x['uyeid']>"0") {
		 		if($_POST['ekle']=="ok" ){
$hangiid=$_GET['ID'];
$degistirs = mysql_query("UPDATE itiraf SET uyeid='".$_POST['uyeid']."' WHERE id='".$hangiid."'"); 


} 
		 ?>  <tr>
         <td width="91" height="33"  class="sod"><strong>Yazar</strong></td>
         <td width="265"  class="sod">
...
if($c==2) { $dos="kadin.png"; }
  ...
  $tuye=$x['uyeid']; if($tuye<"1") { echo $x['rumuz']. ' - <img src="../../../images/'.$dos.'" width="16" height="16" align="bottom" />'; } else { print auye($tuye);   } ?>
    ...
    <td width="444"><table width="407" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="70" height="30"><strong>Baslik</strong></td>
        <td width="337"><input name="baslik" type="text" id="baslik" value="<?= $x['baslik'];?>" size="40" /></td>
        </tr>
    </table></td>
  </tr>
</table></td>
        ...
        <tr><label for="icerik"></label>
          <td><center><textarea name="yorum" id="yorum" cols="90" rows="15" ><?= $x['itiraf'];?></textarea></center></td>
        </tr>
      </table>
      <? if($x['durum']==1) { ?><a href="s-listele.php?o=k&id=<?=$x['id'];?>" class="link">Onayı Kaldır</a><? } else { ?><a href="s-listele.php?o=e&id=<?=$x['id'];?>" class="link">Onayla</a><? } ?> | <span class="z">Tarih: <?= $x['gun']?> <?= ay($x['ay']);?> <?= $x['yil']?>, <?= $x['saat']?> | Kategori: <?= kat($x['katid']);?> | <? if($x['durum']==2) { ?><a href="s-listele.php?o=e&id=<?=$x['id'];?>" class="link">Çöpten çıkar ıyk.s</a><? } else { ?><a href="s-listele.php?o=c&id=<?=$x['id'];?>" class="link">Çöpe fırtlat</a><? } ?></span><br />
