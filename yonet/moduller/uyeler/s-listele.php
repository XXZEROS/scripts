...
<br /><table width="927" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td><a href="s-listele.php">Tümünü Göster</a> | <a href="s-listele.php?durum=2" class="link">İstenmeyen Üyeleri Göster</a> | <a href="s-listele.php?durum=0" >Onaylanmamışları Göster</a> | <a href="s-listele.php?admin=1" class="link">Adminleri Göster</a></td>
  </tr>
</table>

<table width="895" border="0" cellpadding="6" cellspacing="1" class="tabe">
      <tr>
        <td width="184" bgcolor="#E8E8E8"><strong>Yazar</strong></td>
        <td width="645" bgcolor="#E8E8E8" ><form id="form1" name="form1" method="get" action="">
...
if($ip<>"") { $iz="where ip='".$ip."'"; } 
 ...
 if($adminn<>"") { $iz=" where admin='1'"; } 
 if($ara<>"") { $iz=" where mail='".$ara."'"; } 
...
if($c==2) { $dos="kadin.png"; }
  ...
  $tuye=$urun['id']; if($tuye<"1") { echo $urun['rumuz']. ' - <img src="../../../images/'.$dos.'" width="16" height="16" align="bottom" />'; } else { print auye($tuye);   } ?><br />
              <a href="?ip=<?=$urun['ip'];?>" class="link"><?=$urun['ip'];?></a></td>
          </tr>
        </table></td>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td><a href="<?=$linkduz;?>?ID=<?=$urun['id'];?>" class="z"><span style="font-weight: bold">Bölüm</span>: <?php echo $urun['bolum'];?> - <span style="font-weight: bold">Mail</span>: <?php echo $urun['mail'];?>..</a></td>
            </tr>
          <tr>
            <td height="28"><? if($urun['durum']==1) { ?><a href="?o=k&id=<?=$urun['id'];?>" class="link">Onayı Kaldır</a><? } else { ?><a href="?o=e&id=<?=$urun['id'];?>" class="link">Onayla</a><? } ?> | <span class="z">K.T: <?= $urun['ktarih'];?> | İp: <a href="s-listele.php?ip=<?=$urun['ip'];?>"><?=$urun['ip'];?></a> |
<? if($urun['durum']==2) { ?><a href="?o=e&id=<?=$urun['id'];?>" class="link">İstenen Yap</a><? } else { ?><a href="?o=c&id=<?=$urun['id'];?>" class="link">İstenmeyen</a><? } ?></span> ||| <a href="s-listele.php?mailxx=<?php echo $urun['mail'];?>&kadi=<?php echo $urun['kadi'];?>&sifrex=<?php echo $urun['sifre'];?>">Mailine şifresini tekrar gönder</a></td>
            </tr>
        </table></td>
        <td><center>
